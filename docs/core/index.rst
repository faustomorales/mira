...
Second, we need to assign detections to images. To do this, we use the `Scene` object, which consists of an `Image`, a list of `Annotation` and an `AnnotationConfiguration`. Each `Annotation` contains the bounding box and an `AnnotationCategory` to categorize it.
