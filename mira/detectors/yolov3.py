...
YOLO_FILE_CONFIG = {
    "tiny": {
        ...
        "weights_hash": "dccea06f59b781ec1234ddf8d1e94b9519a97f4245748a7d4db75d5b7080a42c",
        "cfg_fname": "yolov3-tiny.cfg",
        "cfg_url": "https://raw.githubusercontent.com/pjreddie/darknet/master/cfg/yolov3-tiny.cfg",
        "cfg_hash": "84eb7a675ef87c906019ff5a6e0effe275d175adb75100dcb47f0727917dc2c7",
    ...
    "full": {
        ...
        "weights_hash": "523e4e69e1d015393a1b0a441cef1d9c7659e3eb2d7e15f793f060a21b32f297",
        "cfg_fname": "yolov3.cfg",
        "cfg_url": "https://raw.githubusercontent.com/pjreddie/darknet/master/cfg/yolov3.cfg",
        "cfg_hash": "22489ea38575dfa36c67a90048e8759576416a79d32dc11e15d2217777b9a953",
...
def make_input_layer(x, num_filters, name):
    """Make the input portion of the network."""
    xi = DarknetConv2D_BN_Leaky(
        filters=num_filters, kernel_size=(1, 1), name=name + "_c2d_bn_leaky1"
    )(x)
    xi = DarknetConv2D_BN_Leaky(
        filters=num_filters * 2, kernel_size=(3, 3), name=name + "_c2d_bn_leaky2"
    )(xi)
    xi = DarknetConv2D_BN_Leaky(
        filters=num_filters, kernel_size=(1, 1), name=name + "_c2d_bn_leaky3"
    )(xi)
    xi = DarknetConv2D_BN_Leaky(
        filters=num_filters * 2, kernel_size=(3, 3), name=name + "_c2d_bn_leaky4"
    )(xi)
    xi = DarknetConv2D_BN_Leaky(
        filters=num_filters, kernel_size=(1, 1), name=name + "_c2d_bn_leaky5"
    )(xi)
    x = DarknetConv2D_BN_Leaky(
        filters=num_filters * 2, kernel_size=(3, 3), name=name + "_c2d_bn_leaky_out"
    )(xi)
    return x, xi
...
def get_backbone(inputs, size):
    ...
    if size == "tiny":
        ...
        conv1 = DarknetConv2D_BN_Leaky(32, (3, 3), name="conv_bn_leaky1")(inputs)
...
def anchors_for_shape(input_shape, anchor_groups, strides):
    ...
    for seeds, stride in zip(anchor_groups, strides):
        steps_y, steps_x = [int(s / stride) for s in input_shape[:2]]
        cx = np.reshape(
            np.arange(start=0, stop=steps_x, dtype="float32"), (1, steps_x, 1, 1)
        ) * (image_w / steps_x)
        cy = np.reshape(
            np.arange(start=0, stop=steps_y, dtype="float32"), (steps_y, 1, 1, 1)
        ) * (image_h / steps_y)
...
def convert_anchors(anchors_xywhs):
    ...
    x1, y1, x2, y2"""
    anchors_xyxy = np.zeros((len(anchors_xywhs), 4))
...
class YOLOv3(Detector):
    ...
    Args:
        ...
        pretrained_top: Whether to use the pretrained full model (only
            ...
            `mira.datasets.COCOAnnotationConfiguration`)
    ...
    def __init__(
        self,
        annotation_config: core.AnnotationConfiguration = datasets.COCOAnnotationConfiguration,
        input_shape: typing.Tuple[typing.Optional[int], typing.Optional[int], int] = (
            None,
            None,
            3,
        ),
        ...
        if pretrained_backbone or pretrained_top:
            ...
            if not os.path.isfile(weights_path):
                log.warning(
                    ...
                    "YOLOv3.make_pretrained_weight_files(sizes=['%s'])"
                    " separately first.",
                    size,
                )
    ...
    def invert_targets(self, y, input_shape, threshold=0.5, **kwargs):
        nms_threshold = kwargs.get("nms_threshold", 0.1)
        ...
        prediction_groups = [
            np.concatenate([p[idx] for p in processed]) for idx in range(batch_size)
        ]
        ...
        for predictions in prediction_groups:
            ...
            for class_index in range(len(self.annotation_config)):
                ...
                indexes = cv2.dnn.NMSBoxes(  # pylint: disable=no-member
                    bboxes=boxes.tolist(),
                    scores=scores,
    ...
    def compute_targets(self, annotation_groups, input_shape):
        ...
        Args:
            ...
            input_shape: The input shape to use for the model.
        ...
        for annotation_group in annotation_groups:
            ...
            for i, group in enumerate(anchors):
                ...
                results[i].append(selected)
                anchorIdx += n_anchors
        return tuple(np.float32(r) for r in results)

    @staticmethod
    def make_pretrained_weight_files(sizes=None):
        """Make pretrained weight files.

        Args:
            sizes: Some subste of ["tiny", "full"]. Defaults to both sizes.
        """
        if sizes is None:
            sizes = ["tiny", "full"]
        for size in sizes:
            ...
            converted_path = os.path.join(
                get_datadir_base(), "weights", "yolov3", config["converted_fname"]
            )
            converted_path_notop = converted_path.replace(
                config["converted_fname"], config["converted_notop_fname"]
            )
            ...
            base_model = get_top(
                ...
                num_classes=len(datasets.COCOAnnotationConfiguration),
            )
