...
def convert_darknet_weights(
    config_path: str, weights_path: str, output_path: str, weights_only: bool = False
):
    ...
    def unique_config_sections(config_file):
        ...
        """
        section_counters = defaultdict(int)  # type: ignore
    ...
    log.info("Weights header: %s, %s, %s, %s}", major, minor, revision, seen)
    ...
    input_layer = tf.keras.layers.Input(
        shape=(None, None, 3), name="input_{0}".format(index["input"])
    )
    index["input"] += 1
    ...
    for section in cfg_parser.sections():
        ...
        if section.startswith("convolutional"):
            ...
            log.info(
                "conv2d (activation: %s, weights: %s, bn: %s)",
                str(activation),
                str(weights_shape),
                str(batch_normalize),
            )
            ...
            if batch_normalize:
                conv_layer = tf.keras.layers.BatchNormalization(
                    ...
                    name="batchnormalization_{0}".format(index["batchnormalization"]),
                )(conv_layer)
            ...
            if activation == "linear":
                ...
                act_layer = tf.keras.layers.LeakyReLU(
                    alpha=0.1, name="leakyrelu_{0}".format(index["leakyrelu"])
                )(prev_layer)
                index["leakyrelu"] += 1
            ...
            if len(layers) > 1:
                log.info(
                    "Concatenating route layers: %s", ",".join([str(l) for l in layers])
                )
            ...
            all_layers.append(
                tf.keras.layers.Add(name="add_{0}".format(index["add"]))(
            ...
            all_layers.append(
                tf.keras.layers.UpSampling2D(
                    stride, name="upsampling2d_{0}".format(index["upsampling2d"])
                )(prev_layer)
            )
    ...
    weights_file.close()
    log.info("Read %s of %s from Darknet weights", count, count + remaining_weights)
    if remaining_weights > 0:
        log.warning("%s unused weights", remaining_weights)
    return model
